...
class LoginManager(object):
    ...
    def reload_user(self, user=None):
        ...
        current_app.logger.info('RELOAD USER:: user: {}'.format(user))

        if user is None:
            user_id = session.get('user_id')
            current_app.logger.info('RELOAD USER:: user_id from session: {}'.format(user_id))
            if user_id is None:
                ctx.user = self.anonymous_user()
            else:
                user = self.user_callback(user_id)
                current_app.logger.info('RELOAD USER:: not anoanymous, getting user: {} by id {}'.format(user, user_id))
                if user is None:
                    current_app.logger.info('RELOAD USER:: cannot getting user by id {}'.format(user_id))
                    logout_user()
                else:
                    ctx.user = user
        else:
    ...
    def _load_user(self):
        ...
        current_app.logger.info('LOAD_USER:: session protection: {}'.format(config.get('SESSION_PROTECTION', self.session_protection)))
        if config.get('SESSION_PROTECTION', self.session_protection):
            deleted = self._session_protection()
            if deleted:
                current_app.logger.info('LOAD_USER:: deleted, reloading user')
        ...
        current_app.logger.info('LOAD_USER:: is user_id missing: {}'.format(is_missing_user_id))
        if is_missing_user_id:
            cookie_name = config.get('REMEMBER_COOKIE_NAME', COOKIE_NAME)
            header_name = config.get('AUTH_HEADER_NAME', AUTH_HEADER_NAME)
            has_cookie = (cookie_name in request.cookies and
                          session.get('remember') != 'clear')
            current_app.logger.info('LOAD_USER:: has cookie: {}'.format(has_cookie))
            if has_cookie:
    ...
    def _load_from_cookie(self, cookie):
        if self.token_callback:
            ...
            current_app.logger.info('LOAD FROM COOKIE:: user_id {}'.format(user_id))
...
def logout_user():
    ...
    '''
    current_app.logger.info('LOGOUT USER:: user_id is session: {}'.format('user_id' in session))
    if 'user_id' in session:
        session.pop('user_id')

    current_app.logger.info('LOGOUT USER:: _fresh is session: {}'.format('_fresh' in session))
    if '_fresh' in session:
        session.pop('_fresh')

    cookie_name = current_app.config.get('REMEMBER_COOKIE_NAME', COOKIE_NAME)
    current_app.logger.info('LOGOUT USER:: cookie_name: {}, exists: {}'.format(cookie_name, cookie_name in request.cookies))
    if cookie_name in request.cookies:
        session['remember'] = 'clear'

    user = _get_user()
    current_app.logger.info('LOGOUT USER:: user: {}'.format(user))
...
def login_required(func):
    ...
    def decorated_view(*args, **kwargs):
        current_app.logger.info('LOGIN_REQUIRED:: user: {}, authenticated: {}'.format(current_user, current_user.is_authenticated()))
